...
from alexa import player, app, config, db, lang, queue, tg, yt
from alexa.helpers import buttons, utils
from alexa.helpers._play import checkUB
from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton
from alexa.helpers.radio import radio_markup



from alexa.helpers.tv import category_markup, fetch_channels


def playlist_to_queue(chat_id: int, tracks: list) -> str:
    """Append playlist tracks and return a compact queue summary."""
    lines = ["<blockquote expandable>"]
    for track in tracks:
        position = queue.add(chat_id, track)
        lines.append(f"<b>{position}.</b> {track.title}")
    summary = "\n".join(lines)
    return summary[:1948] + "</blockquote>"
...
async def play_hndlr(
    ...
    if m.command[0] == "247":
        ...
        try:
            ...
            await player.play_media(chat_id=m.chat.id, message=sent, media=file)
