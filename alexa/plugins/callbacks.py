...
from alexa import player, app, db, lang, queue, tg, yt
from alexa.helpers import admin_check, buttons, can_manage_vc
from alexa.helpers.radio import radio_markup
...
async def _controls(_, query: types.CallbackQuery):
    ...
    if action == "pause":
        ...
        return await player.play_media(chat_id, msg, media)
...
async def radio_callback_handler(_, query: types.CallbackQuery):
    ...
    from alexa.helpers import Track
    ...
    try:
        # Switch the stream
        await player.play_media(chat_id=chat_id, message=query.message, media=media_obj)
...
from alexa.helpers.tv import category_markup, channel_markup, fetch_stream_url, fetch_channels
...
async def tv_channel_callback(_, query: types.CallbackQuery):
    ...
    media_obj.ffmpeg_parameters = "-reconnect 1 -reconnect_at_eof 1 -reconnect_streamed 1 -reconnect_delay_max 2"
    media_obj.headers = {
